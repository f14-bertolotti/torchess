...
__device__ bool enpassant_move(
    ...
    const unsigned char prev_target = boards[env][PREV1_ACTION+2] * 8 + boards[env][PREV1_ACTION+3];
    ...
    const bool is_action_ok = (
        ...
        (boards[env][PREV1_ACTION+4] == 0                                  ) & // previous action was a normal action 
        (boards[env][PREV1_ACTION+3] == actions[env][3]                    ) & // previous action was a to the same column
        (abs(boards[env][PREV1_ACTION] - boards[env][PREV1_ACTION+2]) == 2 ) & // previous action was a double move
